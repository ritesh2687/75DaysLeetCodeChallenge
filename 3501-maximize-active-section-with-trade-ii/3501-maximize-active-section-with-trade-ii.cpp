...
class SparseTable {
public:
    SparseTable(const vector<int>& nums) : n(nums.size()), st(std::bit_width(n) + 1, vector<int>(n + 1)) {
        copy(nums.begin(), nums.end(), st[0].begin());
        for (int i = 1; i <= std::bit_width(n); ++i)
            for (int j = 0; j + (1 << i) <= n; ++j)
                st[i][j] = max(st[i - 1][j], st[i - 1][j + (1 << (i - 1))]);
    }

    int query(unsigned l, unsigned r) const {
        const int i = std::bit_width(r - l + 1) - 1;
    ...
    vector<int> maxActiveSectionsAfterTrade(string s, vector<vector<int>>& queries) {
     const int n = s.length();
        ...
        for (const vector<int>& query : queries) {
            ...
            const int left = zeroGroupIndex[l] == -1 ? -1 : (zeroGroups[zeroGroupIndex[l]].length - (l - zeroGroups[zeroGroupIndex[l]].start));
            const int right = zeroGroupIndex[r] == -1 ? -1 : (r - zeroGroups[zeroGroupIndex[r]].start + 1);

            const int startAdjacentGroupIndex = zeroGroupIndex[l] + 1;
            const int endAdjacentGroupIndex = (s[r] == '1' ? zeroGroupIndex[r] : zeroGroupIndex[r] - 1) - 1;
